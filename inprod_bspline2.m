function prodmat = inprod_bspline2(fdobj1, fdobj2, nderiv1, nderiv2)
...
for j=1:ninterval     
    ...
    % check = polyprod(c1,c2);
    subpolyprod = polyprod2(c1,c2,j);
    if j == 1
        s = size(subpolyprod);
        polyprodmat = zeros(nrep1,nrep2,s(3));
    end
    polyprodmat(j:j+s(1)-1, j:j+s(2)-1, :) = subpolyprod;
    if j > 1
        polyprodmat(j-1:j+s(1)-2, j-1, :) = zeros(1,s(2),s(3));
        polyprodmat(j-1, j-1:j+s(2)-2, :) = zeros(s(1),1,s(3));
    end
    ...
    power = delta;
    prodmati = zeros(s(1),s(2));
    for i=1:D
        prodmati = prodmati + (power/i).*subpolyprod(:,:,D-i+1);
        power = power*delta;
    end
    % add the integral to s
    %prodmat = prodmat + prodmati;
    subprodmat = prodmat(j:j+s(1)-1, j:j+s(2)-1);
    subprodmat = subprodmat + prodmati;
    prodmat(j:j+s(1)-1, j:j+s(2)-1) = subprodmat;
end

%  --------------------------------------------------------------

function convmat = polyprod2(Coeff1, Coeff2,j)
% POLYCONV computes products of polynomials defined by columns of 
%   coefficient matrices Coeff1 and Coeff2

%  Modified by MW to focus on calculating the non-zero sub-block

[po1, norder1] = size(Coeff1);
[po2, norder2] = size(Coeff2);
ndegree1 = po1 - 1;
ndegree2 = po2 - 1;
...
convmat = zeros(po1,po2,N); %AMENDMENT
for i=0:D-1
    ind = (0:i) + 1;
    convmat(:,:,i+1) = Coeff1(ind,j:j+po1-1)'*Coeff2(i-ind+2,j:j+po2-1);
    convmat(:,:,N-i) = Coeff1(D-ind+2,j:j+po1-1)'*Coeff2(D-i+ind,j:j+po2-1);
end
ind = (0:D)+1;
convmat(:,:,D+1) = Coeff1(ind,j:j+po1-1)'*Coeff2(D-ind+2,j:j+po2-1);
